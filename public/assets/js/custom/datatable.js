...
window.VS.mobilePrimaryColumnMeta = window.VS.mobilePrimaryColumnMeta || new WeakMap();

window.VS.cacheMobilePrimaryColumns = function cacheMobilePrimaryColumns(table) {
...
window.VS.mobilePrimaryColumnDefs = function mobilePrimaryColumnDefs(table, mobile) {
  if (!table || !table.classList.contains('vs-mobile-primary')) return [];
  window.VS.cacheMobilePrimaryColumns(table);

  const isMobile = typeof mobile === 'boolean'
    ? mobile
    : window.VS.isMobileTableMode(table);
  ...
  return [
    { visible: true, className: 'all dtr-control', orderable: true, targets: [primary] },
    ...(checkTargets.length ? [{ visible: true, className: 'all vs-mobile-select-cell', orderable: false, targets: checkTargets }] : []),
    { visible: false, className: 'none', targets: detailTargets },
  ];
};

window.VS.mobilePrimaryDetailsHtml = function mobilePrimaryDetailsHtml(table, dt, rowData) {
  window.VS.cacheMobilePrimaryColumns(table);
  ...
  const attrLabel = html => {
    const match = String(html).match(/\b(?:aria-label|title)=["']([^"']+)["']/i);
  ...
  const valueForColumn = index => {
    ...
    if (typeof dataKey === 'string' && Object.prototype.hasOwnProperty.call(rowData, dataKey)) {
  ...
  const html = columns.map((column, index) => {
    ...
    const hasUsefulHtml = /<(img|button|a|span|div|input|select|svg)\b/i.test(raw);
    const label = !text ? attrLabel(raw) : '';
    const value = label
      ? '<span class="vs-mobile-detail-inline">' + raw + '<span>' + escapeHtml(label) + '</span></span>'
      : raw;
    ...
    return '<div class="vs-mobile-detail-item">' +
      '<div class="vs-mobile-detail-label">' + escapeHtml(title) + '</div>' +
      '<div class="vs-mobile-detail-value">' + value + '</div>' +
    '</div>';
  }).join('');
...
window.VS.bindMobilePrimaryDetails = function bindMobilePrimaryDetails(table, getDt) {
...
function initGenericDataTables() {
  ...
  document.querySelectorAll('table.js-data-table').forEach(table => {
    ...
    function parseJsonDataset(name, fallback) {
      ...
      try { return JSON.parse(table.dataset[name]); } catch (e) { return fallback; }
    }
    ...
    function resetChildRows() {
      table.querySelectorAll('tbody tr.parent').forEach(row => row.classList.remove('parent'));
    ...
    function buildDataTable() {
      ...
      const mergedColDefs = [
        ...(noOrderTargets.length ? [{ orderable: false, targets: noOrderTargets }] : []),
        ...mobileColDefs,
        ...extraColDefs,
      ];
      ...
      if (hasPageSearch) {
        const slot = state.dt.table().container().querySelector('.vs-dt-search-slot');
        if (slot) {
    ...
    const breakpoint = window.matchMedia ? window.matchMedia(window.VS.dtMobileQuery) : null;
    if (table.classList.contains('vs-mobile-primary')) {
