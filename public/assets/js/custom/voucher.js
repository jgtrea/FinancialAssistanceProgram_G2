...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function voucherTableMode() {
    return window.VS &&
      window.VS.isMobileTableMode &&
      window.VS.isMobileTableMode(vouchersTable)
      ? "mobile"
      : "desktop";
  }
  ...
  function resetVoucherTableDom() {
    ...
    vouchersTable
      .querySelectorAll("tbody tr.child")
  ...
  function bindVoucherDtEvents() {
    ...
    dt.off(".vsVoucher");
    dt.on(
      "draw.dt.vsVoucher page.dt.vsVoucher search.dt.vsVoucher order.dt.vsVoucher",
      syncPageCheckboxes,
    );
    if (typeof updateSelectAllBanner === "function") {
      dt.on("draw.dt.vsVoucher", function () {
        ...
        if (typeof _fetchingSelectable !== "undefined")
  ...
  function buildVoucherTable(force) {
    ...
    if (datatableUrl) {
      ...
      dt = $(vouchersTable).DataTable({
        ...
        order: [[3, "asc"]],
        dom:
          (customLengthInput ? "" : window.VS.dtHeaderDom(false)) +
          window.VS.dtBodyDom,
  ...
  function updateSelectAllBanner() {
    ...
    selectAllBanner.style.display = "block";
    selectAllBannerText.textContent =
      selSize + " selected. " + selectable + " total matching.";
    selectAllMatchingLink.textContent =
      "Select all " + selectable + " matching across all pages";
  ...
  const voucherBreakpoint = window.matchMedia
  ...
  if (vouchersTable.classList.contains("vs-mobile-primary")) {
    if (voucherBreakpoint && voucherBreakpoint.addEventListener) {
      voucherBreakpoint.addEventListener(
        "change",
        rebuildVoucherTableForBreakpoint,
      );
    ...
    try {
      ...
      trackJob("Exporting", data.status_url, {
        ...
        onError: function (msg) {
          showAlert("Export failed: " + msg, "error");
        },
      });
    } catch (err) {
      console.error(err);
  ...
  if (btnGeneratePdf && pdfForm) {
    async function doGeneratePdf() {
      ...
      try {
        ...
        if (data.queued && data.status_url) {
          ...
          if (toast.setJob)
            toast.setJob({ jobId: data.job_id, statusUrl: data.status_url });
    ...
    btnGeneratePdf.addEventListener("click", function () {
      ...
      if (selectedIds.size > MAX_BATCH) {
        showAlert(
          "You can only print PDFs for up to " +
            MAX_BATCH +
      ...
      try {
        ...
        if (data.queued && data.status_url) {
          trackArchiveJob(data.status_url, data.count || ids.length, {
            ...
            onError: function (msg) {
              showAlert("Archive failed: " + msg, "error");
            },
          });
          return;
        }
