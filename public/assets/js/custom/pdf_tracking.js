...
function pollPdfJob(jobId, statusUrl, toast, onDone) {
  ...
  const tick = async function () {
    ...
    try {
      ...
      if (toast)
        toast.update(jobProgressText("Generating PDF #" + jobId, data));
  ...
  var tick = function () {
    ...
    fetch(statusUrl, fetchOpts)
      ...
      .then(function (data) {
        ...
        if (attempts >= MAX_POLLS) {
          if (opts.onError)
            opts.onError("Still processing. Check back later.", data);
          return;
        }
...
function getPendingDownloadJobs() {
  try {
    var r = localStorage.getItem(DOWNLOAD_JOBS_KEY);
...
function savePendingDownloadJob(job) {
  var list = getPendingDownloadJobs().filter(function (j) {
    return j.jobId !== job.jobId;
  });
  list.push(job);
  try {
    localStorage.setItem(DOWNLOAD_JOBS_KEY, JSON.stringify(list));
  } catch (e) {}
}
function removePendingDownloadJob(jobId) {
  ...
  if (list.length) {
    ...
    localStorage.removeItem(DOWNLOAD_JOBS_KEY);
  }
}
...
function trackJob(verb, statusUrl, opts) {
  ...
  var key =
    opts.persist && opts.jobId
      ? "job-" + opts.jobId
      : "job-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7);
  var toast =
    typeof showPdfToast === "function"
      ? showPdfToast(
          jobProgressText(verb, { progress: { done: 0, total: total } }),
          key,
          null,
          { hideStatus: true, hideDownload: !opts.download },
        )
      : null;
  ...
  getPendingDownloadJobs().forEach(function (job) {
    ...
    trackJob(job.verb || "Working", job.statusUrl, {
      ...
      doneLabel: isDownload
        ? function () {
            return (job.verb || "Export") + " ready — downloading…";
          }
        : function (d) {
...
function trackArchiveJob(statusUrl, count, callbacks) {
  ...
  trackJob("Archiving", statusUrl, {
    ...
    doneLabel: function (data) {
      var n =
        data && data.result && typeof data.result.archived === "number"
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  jobs.forEach(function (job) {
    const toast = showPdfToast(
      "Generating PDF #" + job.jobId + "...",
      "job-" + job.jobId,
      {
        jobId: job.jobId,
